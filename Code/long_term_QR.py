...
class QRLoss(nn.Module):
    ...
    def forward(self, pred, target):
        # pred, target shape: [B, T, N]
        error = target - pred  # 同形状: [B, T, N]
        loss = torch.max((self.q - 1) * error, self.q * error)  # QR公式
        return loss.mean()  # 平均所有值
...
class AdaptiveWeightedMSELoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        weight = 1 + self.beta * torch.exp(self.gamma * error)
        loss = weight * (error ** 2)
...
class Long_Term_Forecast(object):
    ...
    def _select_criterion(self):
        criterion = QRLoss(quantile = 0.95)#0.05下界0.95上界
        return criterion
