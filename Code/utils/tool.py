...
def seq2instance(data, P, Q, slide=1):
    #sliding window
...
def windpowerProcess(args, windpower, train_steps, val_steps, test_steps):
    ...
    trainX = trainX.astype(np.float16)
    trainY = trainY.astype(np.float16)
    valX = valX.astype(np.float16)
    valY = valY.astype(np.float16)
    testX = testX.astype(np.float16)
    testY = testY.astype(np.float16)
...
def loadData_weather(args):
    ...
    trainX, _, valX, _, testX, _ = windpowerProcess(args,wind_power, train_steps, val_steps, test_steps)
