...
class MLP(nn.Module):
    ...
    def forward(self, X, SE=None, TE=None, WE=None):
        ...
        X = X.reshape(batch_size * 134, self.P)
        ...
        # reshape成 (batch, Q, 134)
        X = X.view(batch_size, 134, self.Q).permute(0, 2, 1)
        return X
...
class RLinear(nn.Module):
    def __init__(self, args):
        ...
        self.Linear = nn.ModuleList([
            nn.Linear(args.P, args.Q) for _ in range(134)
        ]) if self.individual else nn.Linear(args.P, args.Q)

        self.dropout = nn.Dropout(args.dropout)
        self.rev = RevIN(134) if self.rev else None
